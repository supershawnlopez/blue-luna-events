...
import { Building2, Sparkles, Clock, Check, ArrowRight } from 'lucide-react'
import { HOMEPAGE_PACKAGES } from '@/lib/config'

export const metadata: Metadata = {
  title: 'Corporate Event Balloon Décor in Tucson, AZ | Blue Luna Events',
  description: 'Brand-colored balloon installations for grand openings, launches, and corporate events in Tucson, AZ. Professional setup, flexible scheduling. Tell us your vision for a personal quote.',
  openGraph: {
    title: 'Corporate Event Balloon Décor in Tucson, AZ | Blue Luna Events',
    description: 'Brand-colored balloon installations for grand openings, launches, and corporate events in Tucson, AZ — custom-built around your vision.',
    url: 'https://bluelunaevents.com/corporate-events',
    images: [{ url: 'https://myumgaqlafbynsgnkdnj.supabase.co/storage/v1/object/public/media/media/1782004577452-gwlt7whlquv.jpeg', width: 1200, height: 630, alt: 'Corporate event balloon décor in Tucson, AZ' }],
  },
  alternates: { canonical: 'https://bluelunaevents.com/corporate-events' },
}

const FEATURES = [
  { icon: Building2, title: 'Brand-Colored Installations', text: 'Balloon garlands, arches, and columns built in your exact brand palette — down to the hex code.' },
  { icon: Sparkles, title: 'Grand Openings & Launches', text: 'An entrance that makes a new location, product, or storefront feel like an event worth stopping for.' },
  { icon: Clock, title: 'Flexible, Off-Hours Setup', text: 'We coordinate delivery and installation around your business hours so setup never disrupts your day.' },
]

const PACKAGES = HOMEPAGE_PACKAGES

const FAQS = [
  { q: 'How much does corporate event balloon décor cost in Tucson?', a: 'It depends on scale — an entrance display reads very differently from a full venue installation. Tell us what you\'re picturing and Monica will personally put together a custom quote.' },
  { q: 'Can you match our brand colors or logo?', a: 'Yes. Share your brand guidelines and we\'ll match the palette exactly — including custom marquee letters or numbers if needed.' },
  { q: 'Do you work with office buildings and venues, and can you set up during off hours?', a: 'Yes. We regularly coordinate with building management and venue staff for early-morning or after-hours access so installation never interrupts business.' },
  { q: 'How far in advance should we book a corporate event?', a: 'We recommend 3–4 weeks for most events, and earlier for large launches or grand openings that need custom brand-matched pieces.' },
]
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2, padding: 'clamp(40px,6vw,80px) 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%,440px),1fr))', gap: '48px', alignItems: 'center' }}>
          <div>
            ...
            <h1 className="font-display" style={{ fontSize: 'clamp(2.5rem,5vw,4.5rem)', fontWeight: 300, lineHeight: 1.05, color: '#0D0F0F', marginBottom: '20px' }}>
              Corporate Event<br/><em style={{ fontStyle: 'italic', color: '#5BBFBF' }}>Décor in Tucson</em>
            </h1>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '1rem', fontWeight: 300, lineHeight: 1.75, color: '#6B7280', maxWidth: '420px', marginBottom: '32px' }}>
              Your launch, grand opening, or company event deserves décor that looks as sharp as your brand.
            </p>
          ...
          <div style={{ position: 'relative', height: 'clamp(260px,40vw,420px)' }}>
            ...
            <div style={{ position: 'absolute', top: '4%', left: '10%', width: '76%', height: '76%', borderRadius: '50%', overflow: 'hidden', border: '4px solid white', boxShadow: '0 16px 44px rgba(13,15,15,0.16)' }}>
              <Image src="https://myumgaqlafbynsgnkdnj.supabase.co/storage/v1/object/public/media/media/1782004577452-gwlt7whlquv.jpeg" alt="Corporate event balloon décor Tucson AZ" fill sizes="(max-width: 900px) 90vw, 45vw" priority style={{ objectFit: 'cover' }} />
            </div>
            <div style={{ position: 'absolute', bottom: '2%', right: '0%', width: '38%', height: '38%', borderRadius: '50%', overflow: 'hidden', border: '4px solid white', boxShadow: '0 10px 32px rgba(13,15,15,0.16)' }}>
              <Image src="https://myumgaqlafbynsgnkdnj.supabase.co/storage/v1/object/public/media/media/1782004575729-6jzg3thaf8.jpeg" alt="Corporate event balloon décor Tucson AZ" fill sizes="(max-width: 900px) 45vw, 20vw" priority style={{ objectFit: 'cover' }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container" style={{ padding: 'clamp(48px,8vw,96px) 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: '48px' }}>
          ...
          <h2 className="font-display" style={{ fontSize: 'clamp(1.8rem,3.5vw,2.8rem)', fontWeight: 300, color: '#0D0F0F' }}>
            Corporate Event <em style={{ fontStyle: 'italic', color: '#3A8F8F' }}>Specialists</em>
          </h2>
        </div>
        ...
        <div id="packages" style={{ scrollMarginTop: '90px' }}>
          <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            ...
            <h2 className="font-display" style={{ fontSize: 'clamp(1.8rem,3.5vw,2.8rem)', fontWeight: 300, color: '#0D0F0F' }}>
              Corporate <em style={{ fontStyle: 'italic', color: '#3A8F8F' }}>Packages</em>
            </h2>
          </div>
        ...
        <div style={{ background: '#0D0F0F', borderRadius: '24px', padding: 'clamp(40px,6vw,64px)', textAlign: 'center', marginTop: '64px' }}>
          <h2 className="font-display" style={{ fontSize: 'clamp(2rem,4vw,3rem)', fontWeight: 300, color: 'white', marginBottom: '16px' }}>
            Let&apos;s Elevate Your <em style={{ fontStyle: 'italic', color: '#5BBFBF' }}>Brand</em>
          </h2>
