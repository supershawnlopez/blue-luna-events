...
function mediaAlt(m: MediaItem) {
  const label = m.event_type ? toLabel(m.event_type) : 'Event'
  return `${label} balloon décor and event styling by Blue Luna Events — Tucson, AZ`
}
              ...
              <div key={m.id} className="gallery-card" onClick={() => openLightbox(i)}
                ...
                {m.type === 'video' ? (
                  <div style={{ position: 'relative', background: '#111', minHeight: '240px' }}>
                    {m.thumbnail_url
                      ? <img src={m.thumbnail_url} alt={mediaAlt(m)} style={{ width: '100%', display: 'block', objectFit: 'cover' }} />
                  ...
                  <div style={{ position: 'relative' }}>
                    <img src={m.url} alt={mediaAlt(m)} style={{ width: '100%', display: 'block', objectFit: 'cover', transition: 'transform 0.45s ease' }} className="gallery-img" />
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(6,7,7,0.97)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            {activeLb.type === 'video' ? (
              ...
              <img key={activeLb.id} src={activeLb.url} alt={mediaAlt(activeLb)}
