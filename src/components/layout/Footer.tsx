    ...
    <footer style={{ background: '#0D0F0F', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%,200px),1fr))', gap: '48px', padding: 'clamp(48px,8vw,80px) 0 48px' }}>
          ...
          <div style={{ gridColumn: 'span 1' }}>
            <Image
              ...
              width={200}
              height={64}
              style={{ height: '52px', width: 'auto', objectFit: 'contain', marginBottom: '18px', display: 'block', filter: 'brightness(0) invert(1)', opacity: 0.9 }}
            />
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.84rem', fontWeight: 300, color: 'rgba(255,255,255,0.4)', lineHeight: 1.75, marginBottom: '22px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              {[
                ...
                <Link key={s.href} href={s.href} target="_blank" aria-label={s.label} style={{
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {[
                { icon: Phone, label: '(520) 222-6142', href: 'tel:5202226142' },
                { icon: Mail, label: 'monica@bluelunaevents.com', href: 'mailto:monica@bluelunaevents.com' },
                { icon: MapPin, label: 'Tucson, AZ', href: undefined },
              ].map((c, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <c.icon size={14} color="rgba(91,191,191,0.6)" style={{ flexShrink: 0 }} />
                  {c.href
                    ? <Link href={c.href} style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.84rem', fontWeight: 300, color: 'rgba(255,255,255,0.5)', textDecoration: 'none' }} className="footer-link">{c.label}</Link>
                    : <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.84rem', fontWeight: 300, color: 'rgba(255,255,255,0.5)' }}>{c.label}</span>
                  }
                </div>
              ))}
            </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', padding: '20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', fontWeight: 300, color: 'rgba(255,255,255,0.2)' }}>
            © 2025 Blue Luna Events · Monica Denogean · Tucson, AZ
          </span>
          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.65rem', fontWeight: 500, color: 'rgba(255,255,255,0.15)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
            BLUELUNAEVENTS.COM
          </span>
        </div>
      </div>
      <style>{`
        .footer-social:hover { border-color: #5BBFBF !important; color: #5BBFBF !important; background: rgba(91,191,191,0.1) !important; }
        .footer-link:hover { color: #5BBFBF !important; }
      `}</style>
    </footer>
  )
}
