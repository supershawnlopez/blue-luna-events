      ...
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%,200px),1fr))', gap: '48px', padding: 'clamp(48px,8vw,80px) 0 48px' }}>
          <div>
            ...
            />
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.84rem', fontWeight: 300, color: '#6B7280', lineHeight: 1.75, marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              {[
                ...
                <Link key={s.href} href={s.href} target="_blank" aria-label={s.label} style={{ width: '48px', height: '48px', borderRadius: '50%', border: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6B7280', transition: 'all 0.2s' }} className="footer-social">
                  <s.icon size={20} />
                </Link>
              ))}
            </div>
          ...
          <div>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.65rem', fontWeight: 600, color: '#9CA3AF', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: '18px' }}>Services</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {['Balloon Décor','Backdrops & Frames','Photo Booth Rental','Audio & MC','All Packages'].map(s => (
                <Link key={s} href="/#packages" style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.88rem', fontWeight: 300, color: '#6B7280', textDecoration: 'none', transition: 'color 0.2s' }} className="footer-link">{s}</Link>
              ))}
            </div>
          </div>
        ...
        <div style={{ borderTop: '1px solid #E5E7EB', padding: '20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', fontWeight: 300, color: '#9CA3AF' }}>© 2026 Blue Luna Events · Monica Denogean · Tucson, AZ</span>
          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.65rem', fontWeight: 500, color: '#9CA3AF', letterSpacing: '0.12em', textTransform: 'uppercase' }}>BLUELUNAEVENTS.COM</span>
        </div>
      </div>
