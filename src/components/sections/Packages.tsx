...
function BookingSheet({ pkg, onClose }: { pkg: typeof PACKAGES[0] | null, onClose: () => void }) {
          ...
          {done ? (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              <div style={{
                ...
                background: 'rgba(91,191,191,0.12)', border: '2px solid #5BBFBF',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                margin: '0 auto 20px',
              }}>
                <Check size={28} color="#5BBFBF" />
              </div>
              <h3 className="font-display" style={{ fontSize: '1.8rem', fontWeight: 400, color: '#0D0F0F', marginBottom: '8px' }}>
                You&apos;re on our list! 🌙
              </h3>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.95rem', fontWeight: 300, color: '#6B7280', lineHeight: 1.6 }}>
                Monica will text you within 2 hours to confirm your date and walk you through the deposit.
              </p>
            </div>
          ) : (
            <>
              {/* Package summary */}
              <div style={{
                background: 'linear-gradient(135deg, #0D0F0F, #1C2222)',
                borderRadius: '16px', padding: '16px',
                display: 'flex', alignItems: 'center', gap: '14px',
                marginBottom: '24px',
              }}>
                <div style={{ width: '56px', height: '56px', borderRadius: '10px', overflow: 'hidden', flexShrink: 0 }}>
                  <Image src={pkg.image} alt={pkg.name} width={56} height={56} style={{ objectFit: 'cover', width: '100%', height: '100%' }} />
                </div>
                <div>
                  <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '10px', fontWeight: 600, color: '#5BBFBF', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '2px' }}>
                    {pkg.name} Package
                  </p>
                  <p className="font-display" style={{ fontSize: '1.5rem', fontWeight: 600, color: 'white', lineHeight: 1 }}>
                    from {pkg.price}
                  </p>
                  <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', color: 'rgba(255,255,255,0.5)', marginTop: '3px' }}>
                    50% deposit · Monica confirms within 2 hrs
                  </p>
                </div>
              </div>

              <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: '1.1rem', fontWeight: 600, color: '#0D0F0F', marginBottom: '6px' }}>
                Reserve Your Date
              </h3>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', fontWeight: 300, color: '#6B7280', marginBottom: '20px' }}>
                Takes 30 seconds. Monica will reach out to confirm everything personally.
              </p>

              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <div>
                  <label className="input-label">
                    <User size={11} style={{ display: 'inline', marginRight: '5px', verticalAlign: 'middle' }} />
                    Your Name
                  </label>
                  <input
                ...
                <div>
                  <label className="input-label">
                    <Phone size={11} style={{ display: 'inline', marginRight: '5px', verticalAlign: 'middle' }} />
                    Phone Number
                  </label>
                  <input
                    className="input-field"
                ...
                <div>
                  <label className="input-label">
                    <Calendar size={11} style={{ display: 'inline', marginRight: '5px', verticalAlign: 'middle' }} />
                    Event Date
                  </label>
                  <input
                    className="input-field"
                ...
                <button
                  ...
                  style={{
                    ...
                    background: loading ? '#9CA3AF' : '#5BBFBF',
                    color: '#0D0F0F', border: 'none',
                ...
                <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.75rem', color: '#9CA3AF', textAlign: 'center', lineHeight: 1.5 }}>
                  No commitment yet — Monica will confirm availability and walk you through next steps.
                </p>
              </form>
            </>
          )}
    ...
    <section id="packages" style={{ padding: 'clamp(64px,10vw,120px) 0', background: '#FDFCFA' }}>
      <div className="container">
          ...
          {PACKAGES.map((pkg, i) => (
            <div
              ...
              style={{
                ...
                transform: pkg.color === 'teal' ? 'scale(1.02)' : undefined,
              }}
            ...
            >
              ...
              <div style={{ padding: '24px' }}>
                <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '10px', fontWeight: 600, color: '#9CA3AF', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '4px' }}>
                  Tier {pkg.tier}
                </p>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', marginBottom: '18px', paddingBottom: '18px', borderBottom: '1px solid #F3F4F6' }}>
                  ...
                  <span className="font-display" style={{ fontSize: '2.4rem', fontWeight: 600, color: '#0D0F0F', lineHeight: 1 }}>{pkg.price}</span>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: pkg.color === 'teal' ? '#5BBFBF' : pkg.color === 'gold' ? 'linear-gradient(135deg, #C9A96E, #E8CCA0)' : 'transparent',
                    color: pkg.color === 'gray' ? '#0D0F0F' : '#0D0F0F',
                    ...
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px',
                    boxShadow: pkg.color === 'teal' ? '0 4px 16px rgba(91,191,191,0.3)' : pkg.color === 'gold' ? '0 4px 16px rgba(201,169,110,0.3)' : 'none',
                    transition: 'all 0.2s',
                  }}
                >
        ...
        <p className="reveal" style={{ fontFamily: 'Inter, sans-serif', textAlign: 'center', marginTop: '28px', fontSize: '0.85rem', fontWeight: 300, color: '#6B7280' }}>
          ...
          <a href="/get-a-quote" style={{ color: '#3A8F8F', borderBottom: '1px solid #3A8F8F' }}>Tell us your vision</a>
          {' '}and we&apos;ll build it. <strong style={{ color: '#0D0F0F', fontWeight: 500 }}>Zelle · Check · Cash accepted.</strong>
        </p>
      </div>
