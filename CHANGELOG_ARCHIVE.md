# CHANGELOG_ARCHIVE.md — Blue Luna Events Older History
### Moved out of `changelog.md` on July 6, 2026 to keep current history readable.
### `SESSION_HANDOFF.md` is the first source of truth for current state.
...
## Note on the gap between this entry and the July 6, 2026 reorg

No changelog entries exist here between May 14, 2026 and July 6, 2026, even though a substantial Studio rebuild happened in that window (StudioNav, My Work overhaul, Social Export, public gallery rebuild, video thumbnail system, Netlify → Vercel migration). That work was tracked in AI session memory but never written back into this repo's own changelog in real time. See `SESSION_HANDOFF.md` and `DECISIONS.md` for what's been reconstructed from that record — verify against actual git history to confirm exact dates and commits.
