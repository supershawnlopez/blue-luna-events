...
- **Not fully verified in-browser this session** — the automated browser tool showed the exact same `readyState 0` for the already-known-working prior hero video too when tested as a control, indicating a tool/automation limitation (not a real regression). Confirm actual playback on a real device before treating this as fully closed.
